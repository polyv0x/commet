...
import 'package:tungstn/utils/error_utils.dart';
...
class UpdateChecker {
  ...
  static Future<void> checkForUpdates() async {
    ...
    const key = "chat.tungstn.published_version";
    ...
    if (fields.containsKey(key)) {
      var date = fields["chat.tungstn.build_date_ms"];

      var val = int.parse(date);
      var time = DateTime.fromMillisecondsSinceEpoch(val);

      var canAutoUpdate = fields["chat.tungstn.auto_update"] == "true";
  ...
  static windowsUpdateAction(BuildContext context, bool canAutoUpdate) async {
    ...
    var installerPath =
        path.join(installPath, "installer", "tungstn-installer.exe");
